...
Principles:
- No hiding losses.
- No instant emotional loss spam.
...
async function fetchSettledSince(since) {
  ...
  const path = `live_signals?select=${selected}&status=eq.settled&settled_at=gte.${encodeURIComponent(since.toISOString())}&order=settled_at.asc&limit=5000`;
...
async function fetchOpenSignals() {
  ...
  const path = `live_signals?select=${selected}&status=eq.open&order=starts_at.asc&limit=5000`;
  return await sbFetch(path, { method: 'GET' });
}
...
function dedupeRows(rows) {
  ...
  for (const row of rows) {
    const key = [
      clean(row.match_name),
      clean(row.strategy_lane),
      clean(row.signal_type),
      clean(row.score_cluster),
      clean(row.selected_side),
    ].join('|');
...
function stats(rows) {
  ...
  return {
    ...
    profit_units: profit,
    flat_roi: settled ? profit / settled : 0,
    avg_odds: avgOdds,
  };
}
...
function compactLines(label, s) {
  return [
    `${label}: ${fmtUnits(s.profit_units)}`,
    `${s.wins}W / ${s.losses}L | Hit: ${fmtPct(s.hit_rate)} | Avg price: ${fmtOdds(s.avg_odds)}`,
  ];
}
...
function paidMessage(roomName, todayRows, rows7, rows30, openCount) {
  ...
  return [
    `🎾 First Set Lab — Proof Vault Update`,
    '',
    header,
    '',
    'Settled window:',
    `${today.settled} signals archived`,
    `${today.wins} wins / ${today.losses} losses`,
    `Flat result: ${fmtUnits(today.profit_units)}`,
    today.settled ? `Avg price: ${fmtOdds(today.avg_odds)}` : 'Avg price: n/a',
    '',
    'Rolling context:',
    ...compactLines('7D', seven),
    ...compactLines('30D', thirty),
    '',
...
function freeMessage(todayRows, rows7, openCount) {
  ...
  return [
    ...
    '',
    'Signals were archived today in the proof ledger.',
    '',
    `Settled today: ${today.settled}`,
    `Rolling 7D flat result: ${fmtUnits(seven.profit_units)}`,
    `Open signals still tracking: ${openCount}`,
    '',
    'The free channel shows proof discipline and education. Core / Quant receive live signal access.',
    '',
...
async function main() {
  ...
  const rooms = [
    { key: 'core', name: 'Core Terminal', chatId: coreChatId },
    { key: 'vip', name: 'Quant Terminal', chatId: vipChatId },
  ];
  ...
  const summary = {
    ...
    send_free_proof: sendFreeProof,
    lookback_hours: lookbackHours,
    all_settled_24h: all24.length,
    all_settled_7d: all7.length,
    all_settled_30d: all30.length,
  ...
  for (const room of rooms) {
    ...
    summary.rooms[room.key] = {
      name: room.name,
      chat_configured: Boolean(room.chatId),
      settled_24h: todayRows.length,
      stats_24h: stats(todayRows),
      stats_7d: stats(rows7),
      stats_30d: stats(rows30),
  ...
  const lines = [
    '# First Set Lab Daily Proof Vault Recap',
    '',
    `Generated: ${summary.generated_at}`,
    `Telegram sending: ${summary.send_telegram}`,
    `Settled 24h: ${summary.all_settled_24h}`,
    `Settled 7D: ${summary.all_settled_7d}`,
    `Settled 30D: ${summary.all_settled_30d}`,
    `Open signals: ${summary.all_open}`,
    `Telegram sent: ${summary.telegram_sent}`,
    '',
    '## Messages',
    ...outputs.flatMap((o) => [`### ${o.room_name}`, '```text', o.message, '```', '']),
    '## Errors',
