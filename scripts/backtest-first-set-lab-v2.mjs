...
const config = {
  ...
  outputDir: process.env.BACKTEST_OUTPUT_DIR ?? 'artifacts/backtests-v2',
};
...
function inferSurface(fixture) {
  const text = `${fixture.tournament_name ?? ''} ${fixture.tournament_round ?? ''} ${fixture.event_type_type ?? ''}`.toLowerCase();
  if (/wimbledon|halle|queen|stuttgart|eastbourne|nottingham|mallorca|hertogenbosch|s-hertogenbosch|bad homburg|birmingham|berlin/.test(text)) return 'grass';
  if (/madrid|rome|monte carlo|barcelona|munich|estoril|geneva|bastad|gstaad|kitzbuhel|kitzbühel|hamburg|umag|marrakech|houston|cordoba|córdoba|buenos aires|rio|santiago|roland garros|french open|palermo|lausanne|bogota|bogotá|ostrava/.test(text)) return 'clay';
  if (/rotterdam|marseille|basel|vienna|stockholm|metz|antwerp|paris masters|bercy|sofia|st petersburg|linz|ostrava indoor/.test(text)) return 'indoor';
  return 'hard';
}
...
function estimatedPlayerStats(side, firstSetEdge = 0, surface = 'hard', level = 'tour_other') {
...
function runModel(matchOdds, fixture) {
  ...
  return { dist: calcSetScoreDist(hold1, hold2, surface), firstSetEdge, hold1, hold2, surface, level };
}
...
function scoreRowsForMatch({ fixture, eventKey, matchOdds }) {
  ...
  const doubles = isDoubles(fixture) ? 'doubles' : 'singles';
  return Object.entries(bookmakerOdds).map(([score, odds]) => {
    ...
    return {
      ...
      score,
      score_family: scoreFamily(score),
...
function summarize(rows) {
  ...
  return {
    ...
    filters: { min_probability: config.minProbability, min_ev: config.minEv, min_edge: config.minEdge, chunk_days: config.chunkDays },
...
function writeOutputs(summary, rows) {
  ...
  const summaryPath = path.join(config.outputDir, `first-set-lab-v2-summary-${stamp}.json`);
  const rowsPath = path.join(config.outputDir, `first-set-lab-v2-rows-${stamp}.csv`);
  fs.writeFileSync(summaryPath, JSON.stringify(summary, null, 2));

  const columns = [
    'event_key', 'match', 'tournament', 'tournament_round', 'tournament_level', 'match_type', 'surface', 'event_date',
    'score', 'score_family', 'odds_bucket', 'actual_first_set_score', 'model_probability', 'fair_odds', 'bookmaker_odds',
    'implied_probability', 'edge', 'expected_value', 'won', 'qualified', 'profit_units', 'first_set_edge', 'hold1', 'hold2',
  ];
...
async function main() {
  console.log('SlipIQ First Set Lab Backtest V2');
  ...
  summary.warning = rows.length === 0
    ...
    : 'V2 adds inferred surface, tournament level, match type, score family, and odds bucket. Surface is heuristic until API/provider supplies official surface.';

  const output = writeOutputs(summary, rows);
  console.log('Backtest V2 summary:');
